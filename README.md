...
- **★ 扫描·审核策略**：执行前先扫描整个数据集，逐标签/字段列出将被去除/假名/保留的项（含脱敏样例与命中数），可逐项调整或新增要去除的标签；"未知"自由文本默认按去除处理（fail-closed）。
- **★ 可信结果**：完成后给 PASS / WARN / FAIL 状态，并**独立回扫输出**核验是否残留真实 PHI（姓名 / ID / 身份证 / 手机号），让你信得过。
...
1. **① 扫描·审核策略**：选**输入文件夹**（**子目录为各患者**的那一层）和**输出文件夹**（空目录）→ 点「扫描」。审核列出的标签（🔴去除 🟠假名 🔵UID重映射 🟢保留；"未知"默认去除），可逐项调整或「＋新增要去除的标签」→「下一步」。
2. **② 执行匿名化**：点「开始匿名化」，逐文件流式显示改了哪些字段 →「查看结果」。
3. **③ 可信结果**：查看 PASS / WARN / FAIL 状态 + 覆盖率 + **独立残留扫描**。输出分两个子目录：`去标识输出_可分享/`（可对外分享）和 `_对照表与报告_请勿分享/`（含 `crosswalk.csv` 回溯钥匙，**切勿随数据分享**）。
